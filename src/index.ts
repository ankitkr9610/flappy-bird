...
const gamePipe = (x:any, y:any) => ({ x, y, checked: false });
const gameSize = 10;
const createPipes = (y:any) =>
  (random =>
  ...
  any >
    (acc =>
      (acc.length < 2 ? [...acc, createPipes(gameSize)] : acc)
        .filter((c:any) => c.some((e:any) => e.y > 0))
        .map((cols:any) => cols.map((e:any) => gamePipe(e.x, e.y - 1))),
    [createPipes(gameSize / 2), createPipes(gameSize)])
);

const fly = (xPos:any) => (xPos > 0 ? (xPos -= 1) : xPos);
const fall = (xPos:any) => (xPos < gameSize - 1 ? (xPos += 1) : gameSize - 1);
...
const updateGame = (bird:any, pipes:any) =>
  (game => (
    pipes.forEach((col:any) => col.forEach((v:any) => (game[v.x][v.y] = 2))),
    (game[bird][0] = 1),
    game
  ))(
    Array(gameSize)
...
const valueOnCollisionFor = (pipes:any) => ({
  when: (predicate:any) =>
      ...
      ((state, [bird, pipes]) => ({
        ...
        lives:
          ...
          valueOnCollisionFor(pipes).when(
            pipes.some((c:any) => c.some((c:any) => c.y === 0 && c.x === bird))
          ),
        score:
